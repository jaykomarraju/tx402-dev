...
import { EvmRpcError, EvmRpcPool, type EvmRpcPoolOptions } from "./rpc.js";
...
/**
 * Creates a stateful adapter.
 *
 * One instance is retained per client so RPC circuit state survives across requests; that
 * state is what `client.resetHealth()` clears.
 */
export function createEvmChainAdapter(options: EvmChainAdapterOptions = {}): ChainAdapter {
  ...
  const poolFor = (networkId: string, network: EvmManifestNetwork): EvmRpcPool => {
    ...
    if (pool === undefined) {
      pool = new EvmRpcPool(network.rpcUrls, options.rpc ?? {});
  ...
  return {
    ...
    async planRoute(request: ChainRouteRequest): Promise<ChainRoute> {
      ...
      const viable = balanceAtomic >= required;
