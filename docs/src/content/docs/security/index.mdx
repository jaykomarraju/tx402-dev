...
transmitted. Same-origin redirects are not followed either in v0.1 — see
[ADR-014](https://github.com/neogeeks/tx402/blob/main/adr/014-paid-retry-redirects-are-not-followed.md)
...
- **Cross-process budget.** The default ledger is in-memory and per-client. Two processes have
  two budgets, and the hourly window resets on restart (ADR-007).
