...
:::caution[Pre-release: you are installing a 0.0.0 placeholder]
The `tx402` published to npm and PyPI is version **`0.0.0`** — an inert name placeholder that
contains no working code. The first functional release is `0.1.0` and is not published yet, so
`npm install tx402` / `pip install "tx402[evm]"` and every `npx tx402` command below pull that
placeholder rather than a working SDK until `0.1.0` ships. Do not depend on `0.0.0`.
:::
