...
| SPEC §4.3 field             | TypeScript                  | Python                          | Default                 |
| :-------------------------- | :-------------------------- | :------------------------------ | :---------------------- |
...
reported with `configPath: "timeouts.initialRequestMs"`.
...
(SPEC §6.7). Setting this very low does not make failures cleaner — it makes ambiguous outcomes
more likely. The initial request carries no such hazard, which is why it is the one that may be
