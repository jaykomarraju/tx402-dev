...
| `logger`                  | no-op                       | An **object** with `debug`, `info`, `warn` and `error`, each taking one event mapping — not a single callback. Receives redacted structured events (SPEC §10); the SDK never writes to the console itself. A value missing any of the four is rejected at construction. |
