...
| `TX402_REDIRECT_BLOCKED`         | `PaidRedirectBlockedError`    | 9    | no        |
...
- **6** — `TX402_SIGNER`
- **7** — `TX402_TRANSPORT`
- **8** — `TX402_PAYMENT_AMBIGUOUS`
- **9** — `TX402_RESOURCE_DELIVERY`, `TX402_REDIRECT_BLOCKED`
...
`PaidRedirectBlockedError` · exit **9** (resource failure) · `retryable: false`
