...
The `tx402` currently on npm and PyPI is version **`0.0.0`** — an inert name placeholder that
contains no working code. The first functional release is `0.1.0`, which is not published yet, so
these install commands (and the `npx tx402` calls below) will not give you a working package until
then. Do not depend on `0.0.0`.
:::
