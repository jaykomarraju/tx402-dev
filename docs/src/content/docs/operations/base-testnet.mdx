...
**opt-in live test**, which SPEC §12.1 requires before release.
...
What **this test file** does not exercise: settlement. ADR-002 puts `/verify` and `/settle` on the
...
`tools/ttv` measures SPEC §16 against. A local merchant does not make the money fake; settlement
