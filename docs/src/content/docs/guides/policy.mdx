...
- **It is not shared across processes.** The default ledger is in-memory and per-client. Two
  processes have two budgets. Supply your own spend store if you need one budget across several.
...
```ts
const state = tx402.getBudgetState({
  policyScope: "api.example.com",
