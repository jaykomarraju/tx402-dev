...
| `info` **or** `warn` | `payment.completed` — `warn` when the merchant supplied no settlement evidence and it committed anyway |
| `warn` **or** `error` | `request.failed` — `warn` when the outcome is ambiguous and the money is still reserved, `error` otherwise |

**Each event is emitted exactly once per request**, at exactly one level. `request.failed` in
particular is emitted from a single place that picks its level from the final disposition, so a
counter over the stream counts requests and never double-counts one. Both languages do this at
the same point and by the same rule.
