...
:::caution[Pre-release: 0.0.0 is a placeholder]
The `tx402` on npm and PyPI is version **`0.0.0`** — an inert name placeholder that contains no
working code. `npx tx402` therefore fetches and runs the placeholder, not a working CLI, until the
first functional release `0.1.0` is published. Do not depend on `0.0.0`.
:::
