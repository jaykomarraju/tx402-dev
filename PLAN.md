...
| S15b | Audit remediation | ⬜ Required | Ordinary development session. Resolve O44–O54 with independently derived regression tests in both languages where applicable; do not combine it with the S15c re-audit. |
| S15c | Audit re-run | ⬜ Required | §11.1: the audit gate is complete only when a separate findings-only re-run comes back clean. S16 must not start before that. |
...
**Normative test status (SPEC §12.2): every T-001 through T-020 is ✅ green as of S14.**
**S15 audit qualification:** that green status is historical gate output, not a release verdict.
O44 and O52 are contradicted by tests and frozen vectors that assert the implementation's
behaviour instead of the governing SPEC/ADR contract; O47 is hidden by workspace-installed peer
dependencies; O48's checks can pass without having performed the claimed verification. S15b and
then a clean S15c are required before S16.
