...
| O34 | **Route selection can flip between two candidates that tie on every ordering key above latency, and it flakes the Python suite under coverage.** Found at S11. `test_selection_is_identical_across_repeated_runs` failed roughly one run in five to one in ten **only with coverage instrumentation enabled** — twelve consecutive `--no-cov` full runs and forty consecutive direct repetitions of the scenario were clean, and the same suite is green without it. Diagnosis: SPEC §6.4 step 18 orders by _viable, preference, fee, health, latency, index_, and `observedLatencyMs` is a fresh wall-clock measurement rather than an input. When the EVM and SVM candidates tie on everything above it, latency decides, and coverage instrumentation adds enough noise to reverse the comparison. **This will flake CI, which always runs coverage.** The frozen vectors are unaffected because they supply fixed latency values, which is why T-016 stays green and hid this. **Not fixed at S11 and deliberately not papered over:** the candidate fixes (quantise latency into buckets before comparing, or demote latency below requirement index) each change route ordering, which is frozen-vector contract surface under O27 and must land in both languages together under the O29 rule. Diagnose and fix as the first task of S12, in TypeScript and Python in one change. | Agent            | 🟥 Open — CI flake, fix first at S12     |
