...
| O40 | **Solana is not yet at Base's level of proof, and the gap is small and now de-risked.** Base has a single-call live suite, a 50/50 volume run, and a **real settled** payment. Solana has a single-call live suite that stops at the signature, a volume leg blocked by O35, and — until S13 — no settled payment ever. This was in the S12 handoff's scope (it named Solana's facilitator fee payer explicitly) and was missed; the user caught it. **S13 proved settlement works**: a Devnet payment verified and settled through `https://x402.org/facilitator`, signature `cC5hoB8y…YphLBzS`, confirmed on chain at slot 481098931 with `err: null`, fee paid by the published fee payer `CKPKJWNdJEqa81x7CkZ14BVPiY6y16Sxs7owznqtWYp5` so the buyer's SOL is untouched. Remaining work, all small: (a) the run was a **self-transfer**, because an arbitrary recipient has no USDC associated token account on Devnet and the transfer fails simulation — create and fund an ATA for a throwaway recipient (~0.002 SOL rent) so the receipt shows value actually leaving, as the Base run does; (b) generalise `tools/ttv` to take a network argument instead of hard-coding Base, and record both numbers; (c) O35 still gates the 50-call volume leg independently. Also found: when settlement fails, `tools/test-merchant` answers 402 **without** a `PAYMENT-REQUIRED` header, so the buyer reports `missing-header` rather than the real cause — a fixture defect that made the first failed Solana attempt harder to read than it needed to be.              | Agent            | 🟥 Open — do at S14                      |
