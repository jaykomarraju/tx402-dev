...
**Current state:** _(updated end of S4)_ **the TypeScript core is complete through M2.** Public money
inputs are parsed directly into atomic-unit `bigint`, the immutable PolicyEngine evaluates in SPEC
§6.3 order, SEC-009 request fingerprints are frozen cross-language, and MemorySpendStore provides
atomic rolling-hour reservations with the required 120-second TTL. T-006/T-007 are green, including
concurrency/property coverage. Both language runners execute all 42 shared vectors through M2.
Routing and signing remain intentionally unwritten and begin with the EVM adapter at M3.
...
| D8  | **ADR-008's total-core size ceiling is 28 KiB from the measured M2 baseline; the independent 25 KiB own-code limit is unchanged.**                                                                                | M2's policy/ledger are necessarily reachable from `createTx402Client`. Measured 25.79 KiB total / 10.99 KiB own; explicit ADR-008 amendment, no dependency change.                                               |
...
**Normative test status (SPEC §12.2):** T-001, T-006, T-007, T-009, T-013, and T-018 are ✅ green.
T-002…T-005, T-008, T-010…T-012, and T-014…T-020 remain ⬜ because their signing, routing,
completion, or release milestones have not landed. The test merchant carries scenarios for T-010,
T-011, T-012, and T-017 so those later tests already have deterministic fixtures.
...
| O4  | Informational size ceiling. **Resolved at S3 and amended at S4:** M1 froze 24 KiB from a 22.28 KiB baseline. Required M2 policy/ledger made the total 25.79 KiB, so ADR-008 explicitly amends the total ceiling to 28 KiB with 8.6 % headroom. The independent own-code gate remains 25 KiB; actual M2 own code is 10.99 KiB.                                                                                                                        | Agent            | ✅ Resolved S4                               |
...
| O15 | Conformance gaps left at M0. **Partially resolved through S4:** oversized-header, three SEC-009 fingerprint goldens, and three spend-ledger transition vectors execute in both languages. Only route-candidate/order vectors remain, due with M5/S7.                                                                                                                                                                                                 | Agent            | 🟨 Only route vectors remain S7              |
| O16 | GitHub Actions' deprecated Node 20 action runtime. **Resolved at S4:** official current majors were available and CI now uses `actions/checkout@v7`, `actions/setup-node@v7`, `pnpm/action-setup@v6`, and immutable `astral-sh/setup-uv@v9.0.0`, all on the Node 24 action runtime. setup-uv's prior cache pruning is explicit. The application matrix still tests Node 20 and 22.                                                                   | Agent            | ✅ Resolved S4                               |
