...
than fails when the Cloudflare token is absent.
...
| S15c | Audit re-run | ⬜ Required | §11.1: the audit gate is complete only when a separate findings-only re-run comes back clean. S16 must not start before that. **The re-audit must derive its cases from the public promises and hostile inputs again, not from S15b's regressions** — a re-run that only replays what the fix session wrote proves the fix session agreed with itself. |
...
re-verified at S15b** against the corrected disposition table — T-012's expected error class
