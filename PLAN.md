...
| O16 | GitHub Actions' deprecated Node 20 action runtime. **Resolved at S4:** official current majors were available and CI now uses `actions/checkout@v7`, `actions/setup-node@v7`, `pnpm/action-setup@v6`, and `astral-sh/setup-uv@v9`, all on the Node 24 action runtime. setup-uv's prior cache pruning is explicit. The application matrix still tests Node 20 and 22.                                                                                 | Agent            | ✅ Resolved S4                               |
